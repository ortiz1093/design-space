...
X = np.loadtxt('ANSYS_Results_2021Mar11.csv', skiprows=1, delimiter=',')
...
N = 1000
X_new = np.vstack((
    np.random.rand(N)*np.diff(Wall) + Wall.min(),
    np.random.rand(N)*np.diff(Width) + Width.min(),
    np.random.rand(N)*np.diff(Height) + Height.min(),
    np.random.rand(N)*np.diff(Floor) + Floor.min(),
    np.random.rand(N)*np.diff(Length) + Length.min()
)).T
